...
class MainActivity : AppCompatActivity() {
    ...
    override fun onBackPressed() = when (findNavController(R.id.nav_host).currentDestination?.id) {
    ...
    private fun initNavigation() = findNavController(R.id.nav_host).run {
        setupActionBarWithNavController(this, appBarConfiguration)
        bottom_nav.setupWithNavController(this)
        addOnDestinationChangedListener { _, destination, _ -> updateViewByDestination(destination.id) }
    }

    private fun updateViewByDestination(destinationId: Int) {
        when (destinationId) {
            R.id.splashFragment -> {
